...
local Linux = {
    Theme = {
        Background = Color3.fromRGB(20, 20, 25), -- Um cinza mais escuro e elegante
        Element = Color3.fromRGB(35, 35, 40), -- Elementos com um tom mais claro para contraste
        Accent = Color3.fromRGB(255, 85, 85), -- Vermelho mais suave e moderno
        Text = Color3.fromRGB(200, 200, 200), -- Texto mais claro para legibilidade
        Toggle = Color3.fromRGB(45, 45, 50), -- Toggle com um tom mais escuro
        TabInactive = Color3.fromRGB(30, 30, 35), -- Abas inativas com contraste
        DropdownOption = Color3.fromRGB(40, 40, 45), -- Opções de dropdown mais destacadas
...
function Linux:Notify(config)
    ...
    Linux.Instance("UICorner", {
        Parent = Notification,
        CornerRadius = UDim.new(0, 8) -- Bordas mais suaves
    })

    -- Adicionando sombra
    Linux.Instance("UIStroke", {
        Parent = Notification,
        Color = Linux.Theme.Shadow,
        Transparency = 0.5,
        Thickness = 2,
    ...
    Linux.Instance("TextLabel", {
        ...
        Font = Enum.Font.GothamBold, -- Fonte mais moderna
    ...
    local tweenInfo = TweenInfo.new(0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut) -- Animação mais suave
...
function Linux.Create(config)
    ...
    local tabWidth = config.TabWidth or 120 -- Aumentei um pouco para dar mais espaço
    local isMobile = InputService.TouchEnabled and not InputService.KeyboardEnabled
    ...
    -- Adicionando sombra ao frame principal
    Linux.Instance("UIStroke", {
        Parent = Main,
        Color = Linux.Theme.Shadow,
        Transparency = 0.5,
    ...
    local TopBar = Linux.Instance("Frame", {
        ...
        Size = UDim2.new(1, 0, 0, 30), -- Aumentei a altura para um visual mais elegante
        ZIndex = 2,
        BorderSizePixel = 0
    })
    ...
    local TitleLabel = Linux.Instance("TextLabel", {
        ...
        Position = UDim2.new(0, 10, 0, 0), -- Mais padding à esquerda
    ...
    local SubtitleLabel = Linux.Instance("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Size = UDim2.new(0, 0, 1, 0),
        Position = UDim2.new(0, 10 + TitleLabel.TextBounds.X + 6, 0, 0),
        Font = Enum.Font.Gotham,
    ...
    local MinimizeIcon = Linux.Instance("ImageLabel", {
        ...
        Image = "rbxassetid://7072718366", -- Ícone mais moderno
    ...
    Linux.Instance("ImageLabel", {
        ...
        Size = UDim2.new(0, 18, 0, 18),
        Position = UDim2.new(0.5, -9, 0.5, -9),
        Image = "rbxassetid://7072721682", -- Ícone de fechar mais moderno
        ImageColor3 = Linux.Theme.Text,
        ZIndex = 3
    })
    ...
    Linux.Instance("UIListLayout", {
        Parent = TabHolder,
        Padding = UDim.new(0, 5), -- Mais espaço entre as abas
    ...
    MinimizeButton.MouseButton1Click:Connect(function()
        ...
        if not isMinimized then
            TweenService:Create(Main, tweenInfo, {Size = UDim2.new(0, 220, 0, 30), Position = UDim2.new(0.5, -110, 0, 0)}):Play()
    ...
    function LinuxLib.Tab(config)
        ...
        local TabBtn = Linux.Instance("TextButton", {
            Parent = TabHolder,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, -10, 0, 32), -- Aumentei a altura para um visual mais elegante
            Font = Enum.Font.Gotham,
            Text = "",
            TextColor3 = Linux.Theme.Text,
            TextSize = 14,
        ...
        -- Adicionando sombra ao botão da aba
        Linux.Instance("UIStroke", {
            Parent = TabBtn,
            Color = Linux.Theme.Shadow,
            Transparency = 0.7,
        ...
        if config.Icon and config.Icon.Enabled then
            TabIcon = Linux.Instance("ImageLabel", {
                ...
                Image = config.Icon.Image or "rbxassetid://7072721682",
                ImageColor3 = Linux.Theme.Text,
                ZIndex = 2
            })
        end
        ...
        local TabText = Linux.Instance("TextLabel", {
            Parent = TabBtn,
            BackgroundTransparency = 1,
            Size = UDim2.new(1, config.Icon and config.Icon.Enabled and -35 or -20, 1, 0),
            Position = UDim2.new(0, config.Icon and config.Icon.Enabled and 35 or 12, 0, 0),
            Font = Enum.Font.Gotham,
            Text = config.Name,
        ...
        Linux.Instance("UIListLayout", {
            ...
            Padding = UDim.new(0, 6), -- Mais espaço entre elementos
            HorizontalAlignment = Enum.HorizontalAlignment.Left,
            SortOrder = Enum.SortOrder.LayoutOrder
        })
        ...
        local function SelectTab()
            ...
            if CurrentTab then
                ...
                local prevContent = prevTab.Content
                local prevTitleFrame = prevTab.TitleFrame

                TweenService:Create(prevContent, tweenInfo, {CanvasTransparency = 1, Position = UDim2.new(-0.1, 0, 0, 0)}):Play()
                TweenService:Create(prevTitleFrame, tweenInfo, {BackgroundTransparency = 1, Position = UDim2.new(-0.1, 5, 0, 0)}):Play()

                task.delay(0.4, function()
                    prevContent.Visible = false
                    prevTitleFrame.Visible = false
                end)
            end
            ...
            TabContent.CanvasTransparency = 1
            TabContent.Position = UDim2.new(0.1, 0, 0, 0)
            TabContent.Visible = true

            TitleFrame.BackgroundTransparency = 1
            TitleFrame.Position = UDim2.new(0.1, 5, 0, 0)
            TitleFrame.Visible = true

            TweenService:Create(TabContent, tweenInfo, {CanvasTransparency = 0, Position = UDim2.new(0, 0, 0, 0)}):Play()
            TweenService:Create(TitleFrame, tweenInfo, {BackgroundTransparency = 0, Position = UDim2.new(0, 5, 0, 0)}):Play()
        ...
        function TabElements.Toggle(config)
            ...
            local function UpdateToggle()
                local tween = TweenInfo.new(0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut)
                if State then
                    ...
                    TweenService:Create(Knob, tween, {Position = UDim2.new(0, 2, 0, 2), BackgroundColor3 = Color3.fromRGB(160, 160, 160)}):Play()
                end
            end
